...
%\begin{equation}
%\sigma_{\bar{x}}^2 =  \sum_{i=1}^k w^2_i \left[ (x_i -\bar{x})^2 + (x_i - y)^2 \right] .
%\end{equation}
...
%\sigma_{\bar{x}_{j, s, t}}^2 =  \sum_{i=1}^k w^2_{i-j, s}  \left[  (x_{i, s, t} - \bar{x}_{j, s, t})^2 + \sigma_{i-j, s}^2 \right].
%\end{equation}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm]{Figures/Var.pdf} % requires the graphicx package
   \caption{Variance of the weighted average of (a) NEP and (b) NBP from CMIP6 ESMs calculated from from equation \ref{eq:avevar}.}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm]{Figures/Time_series_NXP_vf.pdf} % requires the graphicx package
   \caption{Annual time series of (a) NEP and (b) NBP from individual CMIP6 ESMs obtained as the weighted averages using the method proposed here (black lines). Uncertainty ranges are expressed as the average value $\pm$ the full value of $\sigma$ from equation \ref{eq:wapi}.}
