...
A second challenge is that the total number of parameters in a given ESM is usually unknown to the user of ESM numerical output. The classical statistical theory tends to assign penalties to models according to their number of parameters, but this is practically impossible for published output from ESMs because there is no consistent reporting of the type and number of parameters, and whether they were obtained by an optimization method or agnostically inputted. Thus, the classical theory needs to be modified by an approach that disregards model complexity and does not add a penalty for it. 
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm]{Figures/CMIP6_FLUXCOM_weights.pdf} % requires the graphicx package
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm]{Figures/CMIP6_CarboScope_weights.pdf} % requires the graphicx package
...
\codedataavailability{All code to reproduce the results is available at Zenodo (doi:). Original data from CMIP6 model output was obtained from the German Climate Computing Center (DKRZ), openly available at \url{https://cmip-data-pool.dkrz.de/}.} %% use this section when having data sets and software code available
...
\begin{figure}[htbp]
   ...
   \caption{Maps of differences with respect to minimum distances ($\mathcal{A}_m$) of CMIP6 models for the variable NEP and the X-BASE observational product. Small numbers in darker colors indicate smaller distances, representing a larger similarity between ESMs and X-BASE.}
   \label{fig:Delta_NEP}
\end{figure}
...
\begin{figure}[htbp]
   ...
   \caption{Maps of differences with respect to minimum distances ($\mathcal{A}_m$) of CMIP6 models for the variable NBP and the Jena CarboScope observational product. Small numbers in darker colors indicate smaller distances, representing a larger similarity between ESMs and CarboScope.}
   \label{fig:Delta_NBP}
\end{figure}
