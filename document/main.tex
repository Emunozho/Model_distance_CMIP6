...
It has been shown in previous publications that the multi-model average of a variable of interest such as surface air temperature tends to agree better with observations than the predictions of any one single model \textcolor{blue}{(references)}. The arithmetic mean from a set of models gives no consideration about the ability of some models perform better than others. This is equivalent to assuming that each model $g_i$ is weighted equally in their prediction ability ($w_i = 1/g_i$). However, it has been shown that this `model democracy' is inappropriate for multi-model inference in climate science \citep{Knutti2010CC, Knutti2017}. Although we are aware that other approaches for multi-model averaging of ESM output have been proposed before \citep[e.g.,][]{Knutti2017, Ribes2021, Sanderson2015, Giorgi2002, Tebaldi2007, Tebaldi2005}, we are not aware of a previous methodology grounded on information-theory principles. In addition, some previous approaches calculate weights as fixed values for all output of a model, but it is desirable to obtain weights based on the ability of some models to provide better estimates for some spatial regions than others. Our proposed approach operates at the grid-cell level, and therefore produces weights based on the ability of a model to predict the spatial distribution of a variable of interest.
...
For ESMs, the vector $\theta$ not only includes the parameters in the model, but also the set of initial conditions of a model that may lead to different ESM output. 
...
\begin{equation} \label{KLD_expan}
I(f, g) = \int f(x) \log f(x) \dd x - \int f(x) \log (g(x | \theta)) \dd x.
\end{equation}

Notice that each of these terms on the rhs can be considered as an expectation of a probability distribution. Furthermore, because full reality $f$ is a fixed quantity that does not depend on parameters or initial conditions, the first term on the rhs can be considered as a constant. In terms of expectations, equation~\ref{KLD_expan} can be written as

\begin{equation}\label{KLD_expect}
...
One characteristic of $\mathcal{A}$ is that it preserves some of the properties of AIC; it can be viewed as a negative entropy \citep{Akaike1985}, and can be used to compare its value across different models on a relative scale. In other words, one can compute $\mathcal{A}_i$ for a set of models $i \in [1, \dots ,k]$ and rank them according to their relative difference. One of the models in the set would have the minimum value $\mathcal{A}_m$ and can be considered as the model with the minimum distance to the observations. Furthermore, for any other model in the set, we can calculate their difference with respect to this `best' model as
...
\end{equation}
i.e., they are estimates of the relative difference between the expected distance between the model and full reality with respect to the same distance for the model that is closer to full reality (Figure~\ref{fig:concept}). Recall from equation~\ref{KLD_expect} that the expected value for full reality is a constant, therefore its actual value plays no role regarding these values of $\Delta_i$.
...
\end{equation}
A note of caution. The last term inside the square root needs to be specified more precisely due to the space-time coordinate indexing of the data and the models. This aspect will be clarified in the next section. 
...
%\textcolor{blue}{The maps of minimum distances $\mathcal{A}_m$ (Figure~\ref{fig:Am}) show that, assuming observation-based products more accurately represent carbon fluxes, CMIP6 ESMs NEP much more reliably than NBP. Generally, ESMs tend to better carbure carbon fluxes in India, central Africa, southern Australia, eastern Europe, and the eastern United States. In contrast, they perform less accurately in northern Africa, the Arabic Peninsula, the North of China and the center of Africa}.
...
\begin{figure}[t]
   ...
   \caption{Minimum distances ($\mathcal{A}_m$) between (a) NEP from CMIP6 ESMs and X-BASE, and (b) NBP from CMIP6 ESMs and CarboScope. More negative values in darker colors indicates smaller distances, representing larger similarity between the ESMs and the observational products. Note that the numerical scales for a and b are different.}
   \label{fig:Am}
\end{figure}
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm, trim={3.9cm, 2cm, 3.3cm, 2.5cm}, clip]{Figures/CMIP6_FLUXCOM_weights.pdf} % requires the graphicx package
...
\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm, trim={3.9cm, 2cm, 3.3cm, 2.5cm}, clip]{Figures/CMIP6_CarboScope_weights.pdf} % requires the graphicx package
...
However, Figure \ref{fig:averages} only shows the first component of the variance from equation \ref{eq:sigma2}; i.e., only the deviation of the models with respect to the averages excluding the deviation of the models from the observational product. When combining both sources of uncertainty, the overall variance is much larger, but the uncertainty of the weighted average is still consistently lower. 
...
\section{Discussion}
Although other authors have proposed methods to obtain weights and multi-model averages from the predictions of ESMs \citep[and references therein]{Tebaldi2007}, we presented here an approach based on information-theory concepts that is easy to implement and can help to improve inferences from ESMs minimizing biases and reducing uncertainties. 
