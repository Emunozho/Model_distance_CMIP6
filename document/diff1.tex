...
%DIF DEL main.tex     Mon May 19 18:49:00 2025
%DIF ADD mainR1.tex   Sun Aug 10 18:33:16 2025
%% ---------------------------------
...
Inferences in Earth system science (ESS) depend to a large extent on the output from Earth system models (ESMs) that combine different components of the carbon, water, and energy balance of Earth and make spatial and temporal predictions of a large number of variables of scientific interest. These models are relatively large and transcend specific knowledge of single disciplines. Multiple modeling groups run large simulations and participate in coordinated Model Intercomparison Projects (MIP), where the forcings to the model are similar and the outputs across models are comparable \citep{Eyring2016}. For making inferences about a particular aspect of the dynamics of the Earth system, one is confronted with the problem of what model to analyze, how to assign a degree of belief to the results of a certain model, and how to perform averaging across models to obtain an unbiased estimator of some variable of interest \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{Hagedorn2005, Knutti2010CC, Knutti2010JC, Hausfather2022, Tebaldi2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{Giorgi2002, Giorgi2003, Hagedorn2005, Knutti2010CC, Knutti2010JC, Hausfather2022, Tebaldi2007, Sain2025}}\hskip0pt%DIFAUXCMD
}\DIFaddend . 
...
A third challenge is the high-dimensionality of the problem of multi-model inference with ESMs. While the classical theory usually considers one predicted variable and a small set of explanatory variables linked by a polynomial function, the problem \DIFdelbegin \DIFdel{in ESS }\DIFdelend \DIFaddbegin \DIFadd{of inference in Earth system science }\DIFaddend is to obtain expected values of a large set of state variables reported in a multi-dimensional lattice (geographical coordinates, time, and height or depth). 
...
It has been shown in previous publications that the multi-model average of a variable of interest such as surface air temperature tends to agree better with observations than the predictions of any one single model \citep{Doblas-Reyes2003, Hagedorn2005, Elvidge2023}. The arithmetic mean from a set of models gives no consideration about the ability of some models to perform better than others. This is equivalent to assuming that each model is weighted equally in their prediction ability. However, it has been shown that this `model democracy' is inappropriate for multi-model inference in climate science \citep{Knutti2010CC, Knutti2017}. Although we are aware that other approaches for multi-model averaging of ESM output have been proposed before \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Knutti2017, Ribes2021, Sanderson2015, Giorgi2002, Tebaldi2007, Tebaldi2005, Elvidge2023}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[e.g.,][]{Knutti2017, Ribes2021, Sanderson2015, Giorgi2002, Tebaldi2007, Tebaldi2005, Merrifield2020, Elvidge2023}}\hskip0pt%DIFAUXCMD
}\DIFaddend , we are not aware of a previous methodology grounded on information-theoretic principles. In addition, some previous approaches calculate weights as fixed values for all output of a model, but it is desirable to obtain weights based on the ability of some models to provide better estimates for some spatial regions than others. Our proposed approach operates at the grid-cell level and, therefore, produces weights based on the ability of a model to predict the spatial distribution of a variable of interest.
...
We assume that a model $g$ is an approximation of the unknown full reality $f$, and the \emph{\DIFdelbegin \DIFdel{distance}\DIFdelend \DIFaddbegin \DIFadd{divergence}\DIFaddend } between $g$ and $f$ is given by the Kullback-Liebler information
...
\DIFdelbegin \DIFdel{This distance }\DIFdelend \DIFaddbegin \DIFadd{The information or divergence }\DIFaddend $I(f, g)$ \DIFdelbegin \DIFdel{can be interpreted as the }\DIFdelend \DIFaddbegin \DIFadd{is often interpreted as a distance between two distributions even though it does not meet the mathematical definition of a distance metric; i.e. it is not symmetric (the value from $f$ to $g$ can be different than from $g$ to $f$) and it does not satisfy the triangle inequality \mbox{%DIFAUXCMD
\citep{Cover2006}}\hskip0pt%DIFAUXCMD
. 
...
\DIFadd{$I(f, g)$ can also be interpreted from the point of view of information theory as the amount of information available to discriminate $g$ from $f$ \mbox{%DIFAUXCMD
\citep{Kullback1951}}\hskip0pt%DIFAUXCMD
, or as the }\DIFaddend loss of information from full reality by the model \DIFaddbegin \DIFadd{approximation}\DIFaddend . In principle, we are interested in selecting or ranking models according to $I(f, g)$, but in practice, we cannot compute $f(x)$. 
...
\begin{equation}
\log (\mathcal{L}(\hat{\theta} | y) ) \DIFaddbegin \DIFadd{- K }\DIFaddend \approx E_y \ E_x [\log(g(x | \hat{\theta}(y) ))],
\end{equation}
\DIFdelbegin \DIFdel{which is equivalent to }\DIFdelend \DIFaddbegin \DIFadd{where $K$ is a bias-correction term. Akaike found that under certain conditions, the number of parameters in the model is a good approximation to the bias in using the log-likelihood to approximate the expected relative distance between the model and full reality. Therefore, $K$ is generally assumed as the number of parameters in a model. 
...
\end{equation}
where the $\approx$ symbol is used here instead of an equality because \DIFdelbegin \DIFdel{a bias-correction term }\DIFdelend \DIFaddbegin \DIFadd{the exclusion of }\DIFaddend $K$\DIFdelbegin \DIFdel{(number of parameters in the model) used by Akaike is ignored in this expression}\DIFdelend . The key result is that the log-likelihood of a model, configured with parameters and initial conditions consistent with some observed data, is an approximation of its expected distance to full reality.

The non-trivial problem now is to determine a reasonable value of a log-likelihood for a parameterized ESM given some observed data. In contrast to common statistical methods of maximum likelihood estimation (MLE) for simple statistical models, there is \DIFdelbegin \DIFdel{not (to our knowledge) any method for MLE }\DIFdelend \DIFaddbegin \DIFadd{no consistent use of MLE methods }\DIFaddend dealing with the hundreds of parameters in \DIFdelbegin \DIFdel{an }\DIFdelend \DIFaddbegin \DIFadd{a fully coupled }\DIFaddend ESM. Also, there is not really any observed data on the gridded format required for comparison with ESM output because measurements are not performed systematically for all points on the surface of the Earth. What we usually have available is data products that are derived from sparse observations and scaled to the terrestrial surface following some data manipulation technique. 
...
\DIFdel{\hat{\sigma}^2 = \frac{\sum^n \hat{\epsilon}_j^2 }{n},
}\end{displaymath}%DIFAUXCMD
\DIFdel{and $\hat{\epsilon}$ are residuals }\DIFdelend \DIFaddbegin \DIFadd{$\hat{\sigma}^2$ is the mean squared deviation }\DIFaddend between model predictions and some available data product. \DIFaddbegin \DIFadd{They are obtained as
}\begin{equation}
\DIFadd{\hat{\sigma}^2 = \frac{\sum^n \hat{\epsilon}_t^2 }{n},
}\end{equation}
\DIFadd{and $\hat{\epsilon}$ are residuals. }\DIFaddend The lhs in equation \DIFdelbegin \DIFdel{~\ref{assumption} }\DIFdelend \DIFaddbegin \DIFadd{\eqref{assumption} }\DIFaddend is the first component of Akaike's Information Criterion (AIC) for the least-squares case without the correction term $K$. Although not perfect, this approximation to the AIC and the log-likelihood function can be estimated from existing model output from ESMs and some observational data product, provided that both are available for the same spatial and temporal coordinates. Thus, mimicking the definition of AIC, we define for our purposes the distance metric $\mathcal{A}$ as
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \begin{equation} \DIFaddbegin \label{eq:A}
\DIFaddend \mathcal{A} := n \log (\hat{\sigma}^2).
\end{equation} 
...
\begin{equation} \DIFaddbegin \label{eq:Deltai}
\DIFaddend \Delta_i := \mathcal{A}_i - \mathcal{A}_m.
\end{equation}
...
\begin{equation} \DIFaddbegin \label{eq:wi}
\DIFaddend w_i = \frac{\exp (-\frac{\Delta_i}{n})}{\sum_{r=1}^k \exp(- \frac{\Delta_r}{n})}.
\end{equation}
Note that $\sum w_i =1$. 
\DIFaddbegin 

\DIFadd{Combining equation \eqref{eq:wi} with equations \eqref{eq:Deltai} and \eqref{eq:A} and after simplification, we arrive at an expression of the weights in terms of the mean squared deviations
}\begin{equation} \DIFadd{\label{eq:invvarw}
w_i = \frac{1/\hat{\sigma}^2_i}{\sum^k_{r=1} 1/\hat{\sigma}^2_r}.
}\end{equation}
\DIFadd{Thus, the weights can be understood as being based on the inverse of the deviations between model output and data product. This expression for the weights (equation \ref{eq:invvarw}) is identical to the formula for inverse-variance weighting, which is a maximum likelihood estimator for the mean of a statistical population with independent and Gaussian distribution \mbox{%DIFAUXCMD
\citep{Bonamente2022}}\hskip0pt%DIFAUXCMD
. The equivalence between these two expressions for the weights (equations \eqref{eq:wi} and \eqref{eq:invvarw}) emerges only because our choice of distance metric $\mathcal{A}$ to approximate the log-likelihood function, but for other representations of $\mathcal{A}$ these formulas would differ. 
}

\DIFaddend With these weights, we can now proceed to compute averages of model predictions according to our strength of belief in the predictions of each model. 
For a variable of interest $x$, the multi-model average is simply
\begin{equation}
\bar{x} = \sum_{i=1}^k w_i \cdot x_i.
\end{equation}

\DIFdelbegin \DIFdel{The estimate of uncertainty of this weighted average$\bar{x}$ would have to incorporate two components, a variance due to inter-model variability, and a variance of each model with respect to the available data $y$. Thus, }\begin{displaymath}
\DIFdel{\sigma_{\bar{x}}^2 =  \sum_{i=1}^k w_i \left[ (x_i -\bar{x})^2 + (x_i - y)^2 \right] .
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{To obtain the variance of the weighted average, we take advantage of the fact that the variance of an inverse-variance weighted average is equal to the inverse of the deviations \mbox{%DIFAUXCMD
\citep{Bonamente2022, Kanters2022, Rotondi2022}
}\hskip0pt%DIFAUXCMD
}\begin{equation}
\DIFadd{\mathrm{Var}(\bar{x}) = \frac{1}{\sum_{i=1}^k 1/\hat{\sigma}_i^2}.
}\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{A note of caution. The last term of this equation needs to be specified more precisely due to the space-time coordinate indexing of }\DIFdelend \DIFaddbegin \DIFadd{This expression for the variance is consistent with the expression of the weights as in equation \eqref{eq:invvarw} where each individual estimate of the inverse-variance for each model is normalized by their sum across all models. Furthermore, the values $1/\hat{\sigma}^2_i$ have a special interpretation from an information theoretic perspective, they are estimates of Fisher's information for distributions of the exponential family \mbox{%DIFAUXCMD
\citep{Rotondi2022}}\hskip0pt%DIFAUXCMD
. They can be interpreted as the amount of information contributed by model $i$ to }\DIFaddend the \DIFdelbegin \DIFdel{data and the models. This aspect will be clarified in the next section}\DIFdelend \DIFaddbegin \DIFadd{weighted average, so the larger the dispersion of model output from the observations, the lower the contribution of the model to the weighted average}\DIFaddend . 

%DIF > The estimate of uncertainty of this weighted average $\bar{x}$ would have to incorporate two components, a variance due to inter-model variability, and a variance of each model with respect to the available data $y$. Thus,
%DIF > \begin{equation}
%DIF > \sigma_{\bar{x}}^2 =  \sum_{i=1}^k w^2_i \left[ (x_i -\bar{x})^2 + (x_i - y)^2 \right] .
%DIF > \end{equation}
%DIF > A note of caution. The last term of this equation needs to be specified more precisely due to the space-time coordinate indexing of the data and the models. This aspect will be clarified in the next section. 
\DIFaddbegin 

\DIFaddend \section{Implementation with ESM output and gridded data products}
...
This equation can be used to produce \DIFaddbegin \DIFadd{geographical }\DIFaddend maps of residuals between one single model $i$ and data-product $j$ for each point in time. An estimate of the \DIFdelbegin \DIFdel{variance over time}\DIFdelend \DIFaddbegin \DIFadd{mean squared deviation}\DIFaddend , from the first time point $t_0$ until a final time point $t_f$ would be
\DIFdelbegin %DIFDELCMD < 
...
\end{equation}
with $n = h (t_f - t_0)$, i.e., the total number of time points, from $t_0$ to $t_f$ multiplied by the time-step $h$. This equation is used to produce one single map (no time points) of \DIFdelbegin \DIFdel{variances }\DIFdelend \DIFaddbegin \DIFadd{mean squared deviations }\DIFaddend for the grid cells of the model versus the available data product. Now we can compute  the distance metric $\mathcal{A}$ as
\begin{equation}
...
\begin{equation}
w_{i-j, s} = \DIFdelbegin \DIFdel{\frac{\exp (- \frac{\Delta_{i-j, s}}{n})}{\sum_{i=1}^k \exp (- \frac{\Delta_{i-j, s}}{n}) }}\DIFdelend \DIFaddbegin \DIFadd{\frac{\exp (- \frac{\Delta_{i-j, s}}{n})}{\sum_{r=1}^k \exp (- \frac{\Delta_{r-j, s}}{n}) },
\quad \mathrm{or} \quad 
w_{i-j, s} = \frac{1/\sigma^2_{i-j,s}}{\sum^k_{r=1} 1/\sigma^2_{r-j, s}}}\DIFaddend .
\end{equation}
...
\begin{equation}
\bar{x}_{j, s, t} = \sum_{i=1}^{k} w_{i-j, s} \cdot x_{i, s, t}\DIFaddbegin \DIFadd{,
\quad \mathrm{or} \quad
}\bar{x}\DIFadd{_{j, s, t} = \frac{\sum_{i=1}^k  x_{i, s, t} /{\sigma^2_{i-j, s}}}{\sum_{i=1}^k 1/\sigma^2_{i-j, s}}}\DIFaddend .
\end{equation}
The variance \DIFdelbegin \DIFdel{would be the sum of the variance due to variability with respect to data plus variability with respect to the weighted average }\begin{displaymath} \DIFdel{%DIFDELCMD < \label{eq:sigma2}%%%
\sigma_{\bar{x}_{j, s, t}}^2 =  \sum_{i=1}^k w_{i-j, s}  \left[  (x_{i, s, t} - \bar{x}_{j, s, t})^2 + \sigma_{i-j, s}^2 \right].
}\end{displaymath}%DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{of the weighted average is obtained as the sum of the inverse-deviation across models
}\begin{equation}\DIFadd{\label{eq:avevar}
\mathrm{Var}(\bar{x}_{j,s}) = \frac{1}{\sum_{i=1}^k 1/\sigma^2_{i-j, s}}.
}\end{equation}\DIFaddend 
\DIFdelbegin \DIFdel{In some cases, it is useful to analyze the variance due to deviation from the average, i.e., only the first term of equation \ref{eq:sigma2}.
}\DIFdelend 

%DIF > The variance would be the sum of the variance due to variability with respect to data plus variability with respect to the weighted average
%DIF > \begin{equation} \label{eq:sigma2}
%DIF > \sigma_{\bar{x}_{j, s, t}}^2 =  \sum_{i=1}^k w^2_{i-j, s}  \left[  (x_{i, s, t} - \bar{x}_{j, s, t})^2 + \sigma_{i-j, s}^2 \right].
%DIF > \end{equation}
%DIF > In some cases, it is useful to analyze the variance due to deviation from the average, i.e., only the first term of equation \ref{eq:sigma2}.
\DIFaddbegin 
...
Notice that the weights \DIFaddbegin \DIFadd{and the variance of the weighted average }\DIFaddend are fixed over time (not time-dependent), but they can be used to obtain averages \DIFaddbegin \DIFadd{and prediction uncertainties }\DIFaddend that include the time dimension\DIFdelbegin \DIFdel{. One consequence of this is that }\DIFdelend \DIFaddbegin \DIFadd{, even for model output that spans beyond the time interval of the available observations. Thus, the weighted average with interval estimation at one standard deviation can be obtained as \mbox{%DIFAUXCMD
\citep[c.f.][]{Rotondi2022}
}\hskip0pt%DIFAUXCMD
}\begin{equation} \DIFadd{\label{eq:wapi}
\bar{x}_{j, s, t} \pm \sqrt{ \mathrm{Var}(\bar{x}_{j,s}) + \sum_{i=1}^k w_{i-j, s}  (x_{i, s, t} - \bar{x}_{j, s, t})^2 }, %DIF >  Based on eq. 11.83 of Rotondi et al. 2022. 
%DIF > \quad \mathrm{or} \quad
%DIF > \frac{\sum_{i=1}^k  x_{i, s, t} /{\sigma^2_{i-j, s}}}{\sum_{i=1}^k 1/\sigma^2_{i-j, s}} \pm \sqrt{ \frac{1}{\sum_{i=1}^k 1/\sigma^2_{i-j, s}} + \sum_{i=1}^k w_{i-j, s}  (x_{i, s, t} - \bar{x}_{j, s, t})^2 },
}\end{equation}
\DIFadd{with $t \in [t^-_0, f^+_f]$; i.e., with initial times starting before model output and data product overlap ($t^-_0 \leq t_0$) and/or final time after model output and data product overlap ($t^+_f \geq t_f$).
In other words, }\DIFaddend we can take a smaller period of time when the observational product and the model output overlap to obtain the weights, and then use the weights to average across the entire time span of the available model output. 

\DIFaddbegin \DIFadd{As a reference, it is useful to obtain the average and the prediction intervals for the equal-weights scheme, where the average is given by 
}\begin{equation}
\DIFadd{\bar{x}_{s,t} = \frac{\sum_{i=1}^k x_{i,s,t}}{k},
}\end{equation}
\DIFadd{and the prediction interval can be expressed as
}\begin{equation}
\DIFadd{\bar{x}_{s,t} \pm \sqrt{ \frac{ (x_{i,s,t} - \bar{x}_{s,t})^2 }{k}}.
}\end{equation}
\DIFadd{Notice that the prediction intervals for this equal-weighting case do not include a variance term with respect to an observational product. This is simply because it is irrelevant to include this source of variation when the averaging does not take into consideration the existence of any reference observation. However, for comparing the prediction intervals between the equal-weights and the weighted average, the variance $ \mathrm{Var}(\bar{x}_{j,s})$ term in equation \eqref{eq:wapi}  adds an undesired source of variability. To make comparisons on equal footing, we thus use a modified version of the prediction intervals for the weighted average as
}\begin{equation} \DIFadd{\label{eq:wapic2}
\bar{x}_{j, s, t} \pm \sqrt{ \sum_{i=1}^k w_{i-j, s}  (x_{i, s, t} - \bar{x}_{j, s, t})^2 }, 
}\end{equation}
\DIFadd{which is identical to the expression used in other model weighting approaches previously proposed with climate model ensambles \mbox{%DIFAUXCMD
\citep[][p. 212]{Sain2025}}\hskip0pt%DIFAUXCMD
.
}


\DIFaddend \section{Ensemble average of land-atmosphere carbon exchange from CMIP6 models}
...
\begin{figure}[t]
   ...
   \caption{Annual time series of (a) NEP and (b) NBP from individual CMIP6 ESMs obtained as arithmetic averages (purple lines) and the weighted averages using the method proposed here (black lines). Uncertainty ranges are expressed as the average value $\pm$ the value of $\sigma$ including only the \DIFdelbeginFL \DIFdelFL{first }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{second }\DIFaddendFL term from equation \DIFdelbeginFL \DIFdelFL{\ref{eq:sigma2}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{\ref{eq:wapi}}\DIFaddendFL . For the arithmetic average, the weights for the computation of uncertainty are $w_i = 1/8$.}
   \label{fig:averages}
\end{figure}

For both variables, NEP and NBP, the estimate of uncertainty with our proposed approach is also significantly lower than the uncertainty based on equal weights (\DIFdelbegin \DIFdel{Figures~\ref{fig:averages} and }\DIFdelend \DIFaddbegin \DIFadd{Figure }\DIFaddend \ref{fig:averagesFullUncertainty}). We obtained a much lower level of uncertainty for the weighted average because the method applies probabilities or strengths of belief to the different models and grid cells, and therefore this averaging procedure increases confidence in the inferred multi-model average. 

However, \DIFaddbegin \DIFadd{the prediction intervals in }\DIFaddend Figure~\ref{fig:averages} \DIFdelbegin \DIFdel{only shows the first component of the variance from equation \ref{eq:sigma2}}\DIFdelend \DIFaddbegin \DIFadd{do not include the inter-model variance term $ \mathrm{Var}(\bar{x}_{j,s})$ from equation \eqref{eq:wapi}}\DIFaddend ; i.e., only the deviation of the models with respect to the averages excluding the deviation of the models from the observational product. When combining both sources of uncertainty, the overall variance is much larger \DIFdelbegin \DIFdel{, but the uncertainty of the weighted average is still consistently lower than that of the }\DIFdelend \DIFaddbegin \DIFadd{for the weighted average (Figure~\ref{fig:averagesFullUncertainty}). For the arithmetic average, this source of variation is generally not included because it is irrelevant in this context to compare the models with observations, but in case this source of uncertainty would be included, the prediction intervals of the }\DIFaddend arithmetic average \DIFdelbegin \DIFdel{(Figure~\ref{fig:averagesFullUncertainty})}\DIFdelend \DIFaddbegin \DIFadd{would be much larger than for the weighted average}\DIFaddend . 
...
\section{Discussion}
Although other authors have proposed methods to obtain weights and multi-model averages from the predictions of ESMs \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep[and references therein]{Tebaldi2007}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[and references therein]{Tebaldi2007, Merrifield2020}}\hskip0pt%DIFAUXCMD
}\DIFaddend , we presented here an approach based on information-theoretic concepts that is easy to implement and can help to improve inferences from ESMs minimizing biases and reducing uncertainties. 
\DIFdelbegin \DIFdel{Some of the }\DIFdelend \DIFaddbegin \DIFadd{Most previously proposed methods have focused on the problem of probabilistic climate change projection taking into account the variability around the ensamble average \mbox{%DIFAUXCMD
\citep{Giorgi2002, Giorgi2003, Tebaldi2005, Knutti2017}}\hskip0pt%DIFAUXCMD
...
\citep{Greene2006}}\hskip0pt%DIFAUXCMD
. For approaches that use a Bayesian approach \mbox{%DIFAUXCMD
\citep[e.g.][]{Tebaldi2005}}\hskip0pt%DIFAUXCMD
, it is necessary to specify a family of prior distributions for the mean and variance, which adds a layer of complexity and uncertainty for obtaining the model weights.  
}

\DIFadd{As opposed to these previous methods, our approach makes no assumptions of a generalized model and/or prior distributions. 
Additional }\DIFaddend advantages of this method over others include: (1) a theoretical foundation based on concepts from probability and information theory \citep{Akaike1974, Akaike1981, Anderson2007, Burnham2002}. (2) The weights have a relevant interpretation, they are evidence in favor of a model prediction having the smallest distance to full reality, even though comparisons are only performed with respect to an uncertain observational product. \DIFaddbegin \DIFadd{For the particular choice of $\mathcal{A}$ as the logarithm of the mean squared deviations, the weights have a straight forward interpretation: the models with less deviation from the observations contribute more to the weighted average. 
}\DIFaddend (3) \DIFdelbegin \DIFdel{Calculation of weights }\DIFdelend \DIFaddbegin \DIFadd{The calculation of weights is }\DIFaddend based on spatial performance of model output with respect to observations, and not a uniform weight for all grid-cells of a model.

\DIFaddbegin \DIFadd{For the particular choice of the distance metric $\mathcal{A}$ as the logarithm of the mean squared deviation between model output and observational product, we obtained weights that are similar to an inverse-variance weighting scheme. This result is expected because the inverse-variance weighted average is an efficient maximum likelihood estimator of the mean of a statistical population, which corresponds to our choice of the mean squared deviation as a log-likelihood function. But we know that this choice of log-likelihood function is not ideal, and should be replaced if other approaches for MLE are available, which would result in an expression for the weights different than the inverse-variance weights. Nevertheless, the use of inverse-variance weighting is an intuitive and easy approach to apply to ESM ensambles, and it is also common in other fields such as in meta-analyses \mbox{%DIFAUXCMD
\citep{Hartung2008, Kanters2022} }\hskip0pt%DIFAUXCMD
and in biomedical studies \mbox{%DIFAUXCMD
\citep{Mansournia2016}}\hskip0pt%DIFAUXCMD
. 
}

\DIFaddend However, as with other approaches, some weaknesses should be acknowledged and should be improved in future research. These are: first, lack of a log-likelihood function for the assessment of model-observation distances. Although such a function \DIFdelbegin \DIFdel{would be unrealistic }\DIFdelend \DIFaddbegin \DIFadd{is difficult }\DIFaddend to obtain given the process of development and parameterization of ESMs, it is still desirable to obtain an unbiased estimator of the log-likelihood of a parameterized model with respect to available data. We are not aware of another method that could be used to replace the simple log of square residuals used here, but it is also important to point out that other measures of distance used in other methods apply mostly the squared residuals as a distance metric. Therefore, our method offers a small theoretical improvement over previous approaches based on the theoretical knowledge that, under certain assumptions, the logarithm of square residuals is a first-order approximation to a maximum likelihood estimator. 

Second, other authors have raised concerns over the issue of model independence \citep{Knutti2010CC, Knutti2010JC}, a problem that we do not address explicitly here. They argue that many models share the same base code or are based on the same underlying principles, and cannot be treated as completely independent estimates for obtaining an unbiased average. In particular, the method of \citet{Knutti2017} produces weights that penalize a model according to its prediction distance to that of other models. We think this is a valid concern, particularly when weights are obtained for the aggregated (sums or averages) across all grid cells of a model. However, the implementation of similar processes in two models but with differences in other components such as its climate sensitivity may lead to very different predictions. For instance, CESM2 and NorESM2-LM share the same land vegetation model, CLM5 (Table~\ref{tab:models}). Although the spatial distribution of the weights for NBP with these models tends to correlate well, the correlation is not uniform across all grid cells and is mostly below 90\% (Figure~\ref{fig:corr}). This implies that a shared component of a model can interact with other non-shared components\DIFdelbegin \DIFdel{of each model}\DIFdelend , resulting in different predictions that should be treated differently for the calculation of weights. 
...
It is also important to note that, in the ideal case in which we would have a perfect understanding of Earth system processes, all mathematical models representing these processes would converge to the same predictions. Therefore, it is still debatable whether models that agree with each other because they have a common representation of underlying processes should be penalized. For this reason, we refrain from introducing a penalization term to our computation of weights, but we acknowledge that this is an issue that deserves more theoretical work. \DIFaddbegin \DIFadd{In particular, from an information-theoretic point of view, the problem should be a dressed in terms of the mutual information shared among models \mbox{%DIFAUXCMD
\citep{Majhi2023}}\hskip0pt%DIFAUXCMD
. 
}\DIFaddend 
...
\begin{figure}[htbp]
   \centering
   \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=14cm]{Figures/Time_series_NXP.pdf} %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=14cm]{Figures/Var.pdf} \DIFaddendFL % requires the graphicx package
   \caption{\DIFdelbeginFL \DIFdelFL{Annual time series }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Variance }\DIFaddendFL of \DIFaddbeginFL \DIFaddFL{the weighted average from the CMIP6 ESMs outputs of }\DIFaddendFL (a) NEP \DIFaddbeginFL \DIFaddFL{using X-BASE }\DIFaddendFL and (b) NBP \DIFdelbeginFL \DIFdelFL{from individual CMIP6 ESMs obtained as arithmetic averages (purple lines) and the weighted averages }\DIFdelendFL using \DIFdelbeginFL \DIFdelFL{the method proposed here (black lines)}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Jena CarboScope}\DIFaddendFL .
\DIFdelbeginFL \DIFdelFL{Uncertainty ranges are expressed as the average value $\pm$ the full value of $\sigma$ }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{The variance is calculated }\DIFaddendFL from equation\DIFdelbeginFL \DIFdelFL{\ref{eq:sigma2}}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{~\eqref{eq:avevar}}\DIFaddendFL .\DIFdelbeginFL \DIFdelFL{For the arithmetic average, the weights for the computation of uncertainty are $w_i = 1/8$.}\DIFdelendFL }
   \DIFaddbeginFL \label{fig:variance_average}
\end{figure}


\begin{figure}[htbp]
   \centering
   \includegraphics[width=14cm]{Figures/Time_series_NXP_vf.pdf} %DIF >  requires the graphicx package
   \caption{\DIFaddFL{Annual time series of  (a) NEP from CMIP6 ESMs and X-BASE, and (b) NBP from CMIP6 ESMs and the Jena CarboScope product obtained as the weighted averages using the method proposed here (black lines). Uncertainty ranges are expressed as the average value $\pm$ the full value of prediction uncertainty from equation \eqref{eq:wapi}.}}
   \DIFaddendFL \label{fig:averagesFullUncertainty}
\end{figure}
